...
import type { IPtyOpenOptions, IWindowsPtyForkOptions } from "./interfaces";
...
export class WindowsTerminal extends Terminal {
  ...
  public constructor(file?: string, args?: ArgvOrCommandLine, opt?: IWindowsPtyForkOptions) {
    super(opt);
    ...
    this._socket.on("ready_datapipe", () => {
      ...
      this._socket.on("error", (err) => {
        ...
        if ((err as any).code) {
          if (~(err as any).code.indexOf("errno 5") || ~(err as any).code.indexOf("EIO")) return;
        }
  ...
  private _deferNoArgs(deferredFn: () => void): void {
