...
import type { IProcessEnv, IPtyForkOptions, ITerminal } from "./interfaces";
...
export abstract class Terminal implements ITerminal {
  ...
  public get onExit(): IEvent<IExitEvent> {
  ...
  public constructor(opt?: IPtyForkOptions) {
    // for 'close'
  ...
  protected _checkType<T>(name: string, value: T | undefined, type: string, allowArray = false): void {
    if (value === undefined) {
      return;
    }
    if (allowArray) {
