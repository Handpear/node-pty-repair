...
class TestTerminal extends Terminal {
  public checkType<T>(name: string, value: T, type: string, allowArray = false): void {
...
describe("Terminal", () => {
  describe("constructor", () => {
    it("should do basic type checks", () => {
      assert.throws(() => new (terminalCtor as any)("a", "b", { name: {} }), "name must be a string (not a object)");
    });
  });
  ...
  describe("automatic flow control", () => {
    it("should respect ctor flow control options", () => {
      const pty = new terminalConstructor(SHELL, [], {
        ...
        flowControlPause: "abc",
        flowControlResume: "123",
      });
